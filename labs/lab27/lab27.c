...
void botton_up(Min_Heap* heap, int i){
    while (i > 0) {
        int pai = (i - 1) / 2;
        if (heap->heap[i].priority < heap->heap[pai].priority ||
...
void get_min_heap(Min_Heap* heap) {
    if (heap->size == 0) {
        ...
        Heap min_element = heap->heap[0];
        printf("prioridade minima %d, chave %u\n", min_element.priority, min_element.key);
    }
}

int main() {
    Min_Heap* heap = create_empty_heap();
    unsigned key;
    char opcao;
    int priority;
    int timestamp = 0;
